...
|---|---|---|
| **Database** | MariaDB / MySQL | Gồm 21 bảng quan hệ tối ưu hóa cùng 1 View tổng hợp điểm tích lũy của khách hàng. |
...
│   ├── 01_schema.sql           # Cấu trúc 21 bảng và 1 view
...
│   ├── create_order.php        # API chính xử lý Tạo đơn hàng (Transaction & Trừ kho)
...
2. **Giao dịch Toàn vẹn (Database Transactions):**
   ...
   * Mọi thao tác bao gồm: Tạo đơn (`orders`) → Thêm món (`order_item`) → Thêm tùy chọn (`order_item_modifier`) → Ghi nhận thanh toán (`payment`) → Trừ tồn kho (`ingredient`) → Tích lũy điểm khách hàng (`loyalty_transaction`) → Cập nhật bàn (`dining_table`) → Ghi nhật ký thao tác (`audit_log`) phải cùng thành công, hoặc cùng bị phục hồi (Rollback) nếu xảy ra lỗi giữa chừng để tránh rò rỉ dữ liệu.
